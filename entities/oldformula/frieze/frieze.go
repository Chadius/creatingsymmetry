...
// Calculate applies the Frieze oldformula to the complex number z.
...
// Symmetry notes the kinds of symmetries the oldformula contains.
...
//AnalyzeForSymmetry scans the oldformula and returns a list of symmetries.
...
// NewFriezeFormulaFromYAML reads the data and returns a Frieze oldformula from it.
func NewFriezeFormulaFromYAML(data []byte) (*Formula, error) {
	return newFriezeFormulaFromDatastream(data, yaml.Unmarshal)
}

// NewFriezeFormulaFromJSON reads the data and returns a Frieze oldformula from it.
